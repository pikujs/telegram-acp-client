...
from telegram_acp_client.bot.messaging import send_safe_message, safe_edit, send_split_diff
from telegram_acp_client.bot.formatting import is_approval_option, escape_markdown, format_diff
...
class PermissionNode:
    ...
    """
    def __init__(self, context, chat_id, session_id, tc_id, tc_idx, options, thread_id=None):
            ...
            btns.append(
                [
                    InlineKeyboardButton(
                        f"{emoji} {opt.name}",
                        callback_data=("perm", str(self.session_id), str(self.tc_idx), opt.option_id),
                    )
                ]
            )
    ...
    def _extract_diff(self, tool_call: Any, node_state: Any) -> str:
        ...
        for content_list in content_sources:
            ...
            for item in (content_list if isinstance(content_list, list) else [content_list]):
                c_type = getattr(item, "type", None) or (item.get("type") if isinstance(item, dict) else None)
                if c_type == "diff":
                    old = getattr(item, "oldText", getattr(item, "old_text", "")) or (item.get("oldText") if isinstance(item, dict) else "")
                    new = getattr(item, "newText", getattr(item, "new_text", "")) or (item.get("newText") if isinstance(item, dict) else "")
        ...
        if ri:
            path = ri.get("path") or ri.get("file_path") or ri.get("filePath") or ri.get("filepath") or "file"
            if "diff" in ri:
                diff_text = str(ri.get("diff"))
                # Clean up Index/=== headers if it's a full patch
                lines = [l for l in diff_text.splitlines() if not (l.startswith("Index: ") or l.startswith("======"))]
                return "\n".join(lines).strip()
            
            old = ri.get("oldText") or ri.get("oldString") or ri.get("old_string") or ri.get("old_text")
            new = ri.get("newText") or ri.get("newString") or ri.get("new_string") or ri.get("new_text")
    ...
    async def handle_click(self, opt_id: str, session=None):
        ...
        is_approved = (
            is_approval_option(opt_name) or 
            is_approval_option(opt_id) or 
            is_approval_option(opt_kind)
        )
        ...
        if is_approved:
            ...
            if not self.future.done():
                self.future.set_result(
                    PermissionOption(option_id=opt_id, name="Allowed", kind="allow_once")
                )
            await safe_edit(self.message, f"{original_text}\n\n✅ *Granted*", parse_mode="Markdown")
        else:
            if not self.future.done():
